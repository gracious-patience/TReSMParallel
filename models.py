...
class Net(nn.Module):
	...
	def forward(self, x):
		self.pos_enc_1 = self.position_embedding(torch.ones(1, self.dim_modelt, 7, 7).to(x.device))
		...
		return predictionQA, consistloss.unsqueeze(0)


class TReS(object):
	
	def __init__(self, config,  svPath, datapath, train_idx, test_idx,Net):
		...
		self.net = Net(config).to("cuda:0") 

		# load checkpoint, change architecture and freeze internal params
		if not config.single_channel:
			if config.finetune:
				# load checkpoint,
				self.net.load_state_dict(torch.load(config.ckpt, map_location="cuda:0"))
				# change architecture
		...
		# Multi-GPU
		self.net = torch.nn.DataParallel(self.net, device_ids=range(config.ngpus)) 		
	...
	def train(self,seed,svPath):
		...
		for epochnum in range(self.epochs):
			...
			for img, label in pbar:
				img = torch.as_tensor(img).requires_grad_(False)
				label = torch.as_tensor(label.to("cuda:0")).requires_grad_(False)
				...
				loss = loss_qa + closs.mean() + loss_qa2 + closs2.mean() + 0.5*( self.l1_loss(tripletlosses,ftripletlosses.detach())+ self.l1_loss(ftripletlosses,tripletlosses.detach()))+0.05*(tripletlosses+ftripletlosses)
